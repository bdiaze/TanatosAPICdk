...
namespace TanatosAPI.Helpers {
	public class HermesHelper(VariableEntornoHelper variableEntorno, ApiKeyHelper apiKey) {
		private readonly string _appName = variableEntorno.Obtener("APP_NAME");
		...
		public async Task<SalHermesEnviar> EnviarCorreo(EntHermesCorreoEnviar correo) {
			...
			HttpResponseMessage response = await client.PostAsync(_hermesBaseUrl + $"Correo/Enviar/{Uri.EscapeDataString(_appName)}", new StringContent(JsonSerializer.Serialize(correo, AppJsonSerializerContext.Default.EntHermesCorreoEnviar), Encoding.UTF8, "application/json"));
		...
		public async Task<SalHermesEnviar> EnviarWhatsapp(EntHermesWhatsappEnviar whatsapp) {
			...
			HttpResponseMessage response = await client.PostAsync(_hermesBaseUrl + $"Whatsapp/Enviar/{Uri.EscapeDataString(_appName)}", new StringContent(JsonSerializer.Serialize(whatsapp, AppJsonSerializerContext.Default.EntHermesWhatsappEnviar), Encoding.UTF8, "application/json"));
			if (response.StatusCode != HttpStatusCode.OK) {
