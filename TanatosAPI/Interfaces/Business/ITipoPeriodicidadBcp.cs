...
namespace TanatosAPI.Interfaces.Business {
	public interface ITipoPeriodicidadBcp {
		...
		public Task<TipoPeriodicidad> Crear(long id, string nombre, string? descripcion, string? cron, int? frecuenciaDias, int? deltaDias, int? deltaMeses, int? deltaAnnos, bool vigencia, NpgsqlTransaction? transaction = null);
